...
# Iterate cluster_tolerance values for benchmark.launch.py and summarize detections.
# - Each run uses run_seconds=10 by default
# - Per-run logs: result-ct-<tol>.log (kept)
# - Summary CSV: ct-vs-bench.csv
...
# Fixed parameters for this sweep (can override via env)
VOXEL_SIZE="${VOXEL_SIZE:-0.2}"
DISTANCE_THRESHOLD="${DISTANCE_THRESHOLD:-0.15}"

# Grid of cluster tolerances (meters)
TOLS=(0.30 0.50 0.70 0.90)

CSV="ct-vs-bench.csv"
echo "cluster_tolerance,real_s,exit_code,pkw,mensch,fahrradfahrer,lkw,total" > "${CSV}"

for tol in "${TOLS[@]}"; do
  CT="${tol}"
  LOG="result-ct-${CT}.log"

  {
    echo "=== Start ct=${CT} ==="
    date -Is
    echo "cmd: ros2 launch ouster_cpp benchmark.launch.py bag=\"${BAG_PATH}\" rate=${RATE} start_offset=${START_OFFSET} run_seconds=${RUN_SECS} voxel_size=${VOXEL_SIZE} distance_threshold=${DISTANCE_THRESHOLD} cluster_tolerance=${CT} ${RMW_ARG[*]:-}"
  ...
  echo "${CT},${real_time:-0},${rc:-0},${pkw:-0},${mensch:-0},${fahr:-0},${lkw:-0},${total:-0}" >> "${CSV}"

  echo "[INFO] ct=${CT} -> pkw=${pkw} mensch=${mensch} fahr=${fahr} lkw=${lkw} total=${total} (real=${real_time}s, rc=${rc})"
done

echo "Résumé écrit dans ${CSV}. Logs conservés (result-ct-*.log)."
