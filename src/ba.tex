...
\noindent
Die vorliegende Arbeit entwickelt eine modular aufgebaute Verarbeitungsstrecke zur LiDAR-basierten Umfelderkennung für das Projektautomobil \textit{CarCeptionX}. Ausgangspunkt ist die bestehende ROS\,2-Messkette des Ouster~OS1, die im Rahmen dieser Arbeit um eine robuste Folge von Vorverarbeitungs-, Segmentierungs-, Cluster- und Trackingbausteinen erweitert wird. Ziel ist eine echtzeitfähige Erkennung relevanter Verkehrsteilnehmer ab einer Mindestgröße von \SI{0.5}{\meter} bei mindestens \SI{90}{\percent} Detektionsrate bei \SI{10}{\hertz} Ausführung.

Das System umfasst eine rausch- und ausreißerreduzierende Vorverarbeitung, eine szenarienunabhängige Bodensegmentierung, die Bildung konsistenter Cluster mit zugehörigen Bounding-Boxen sowie ein darauf aufbauendes Objekt-Tracking. Sämtliche Schritte werden in ROS~2 implementiert, mit einheitlichen Schnittstellen versehen und hinsichtlich Ausführungsfrequenz, Latenz und Speicherbedarf untersucht. Für zentrale Algorithmen wie RANSAC-Bodenschätzung, Euclidean-Cluster-Extraction, DBSCAN und CV-Kalman-Filter werden Parameterstudien durchgeführt, um die Balance zwischen Genauigkeit und Rechenaufwand zu optimieren.
...
Voxelgröße, RANSAC-Abstand/Iterationen, Cluster-Toleranz und Clustergrößen via Launch oder \texttt{ros2 param set} ohne Neustart \\[0.15cm]
\hline
...
ROS-Themen mit dokumentierten Typen (\texttt{sensor\_msgs/PointCloud2}, \texttt{vision\_msgs/Detection3DArray}); Konsolenstatus $\leq 1$ Zeile/s \\[0.15cm]
\hline
...
Darstellung von Rohdaten, Bodenfilter, Clustern und Tracks in RViz2/GUI mit $\geq 10\,\text{FPS}$ \\[0.05cm]
\hline
...
Das \textit{Random Sample Consensus} (RANSAC) bietet eine robuste Ebenenabschätzung, indem es Ausreißer konsequent ignoriert und lediglich Punkte berücksichtigt, die innerhalb eines orthogonalen Abstands zur Modellhypothese liegen.
...
Aufgrund dieses Gleichgewichts zwischen Genauigkeit und Implementierungskomplexität dient RANSAC in dieser Arbeit als Grundlage der Bodensegmentierung.
...
Diese Infrastruktur bildet die Grundlage für die Integration des Ouster~OS1 und kann ohne Bruch der bisherigen Bedienphilosophie erweitert werden. Für eine vollständige Einbindung sind jedoch noch zusätzliche Bausteine nötig, die im ursprünglichen Umfang nicht vorgesehen waren:
...
\begin{enumerate}
    ...
    Diese Nodes müssen im \texttt{sensor.launch.xml}, in den Aufzeichnungsskripten (\texttt{run\_aufzeichnung}) sowie in der MATLAB-App (\texttt{LiDAR\_Embedded\_PC.mlapp}) hinterlegt werden.
...
Die Integration des neuen Verarbeitungspakets \texttt{ouster\_cpp} in die bestehende IFZN-GUI vereinte mehrere gezielte Anpassungen an Dateistruktur, Workspaces und Startmechanismen der ROS\,2-Nodes. Ziel war es, die gesamte Verarbeitungskette -- bestehend aus den Modulen \emph{CropBox}, \emph{VoxelGrid}, \emph{RANSAC-Bodensegmentierung}, \emph{Cluster-Extraktion} und \emph{Tracking} -- in allen vorgesehenen Betriebsmodi nutzbar zu machen:
(1) Laptop,  
(2) Laptop mit Sensor,  
...
Die vorliegende Arbeit entwickelte eine LiDAR-basierte Verarbeitungspipeline, die alle Schritte von der Vorverarbeitung bis zur
Objektverfolgung integriert. Ausgehend von einer ROS~2-basierten Systemarchitektur wurden zunächst robuste Verfahren zur
...
hochaufgelösten LiDAR-Daten ermöglicht.
\section{Ausblick}
...
gemeinsame Objektmerkmale oder BEV-Repräsentationen. Ergänzend sollte geprüft werden, wie SLAM-Verfahren auf Basis der
LiDAR-Daten eine lokale Karte aufbauen und die Eigenbewegung präzisieren können, um die Objekttrajektorien in ein konsistentes
...
unterschiedliche Fahrzeugplattformen und Einsatzgebiete zu sichern und das volle Potenzial der LiDAR-basierten
