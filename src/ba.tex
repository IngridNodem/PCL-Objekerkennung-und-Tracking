...
Dieses Kapitel bündelt die wissenschaftlichen Grundlagen der \ac{LiDAR}-Sensorik, verortet sie im automobilen Kontext und leitet daraus die für die Verarbeitungskette relevanten Anforderungen ab. Es beschreibt die Messprinzipien, skizziert den historischen Entwicklungspfad, grenzt \ac{LiDAR} gegenüber Kamera und Radar ab und erläutert den Aufbau des verwendeten Sensors.

Moderne Fahrerassistenzsysteme und autonome Fahrfunktionen stützen sich auf ein Zusammenspiel aus Kamera-, Radar- und \ac{LiDAR}-Sensorik. Kameras liefern hochauflösende Bildinformationen für semantische Aufgaben, Radar überzeugt durch robuste Distanz- und Geschwindigkeitsmessung auch bei ungünstigen Wetterbedingungen. \ac{LiDAR} ergänzt diese Systeme durch präzise dreidimensionale Punktwolken mit hoher Winkelauflösung und konstanter Leistungsfähigkeit bei wechselnden Lichtverhältnissen \parencite{beuth2021handbuch}.

Das Akronym \emph{\ac{LiDAR}} bezeichnet optische Messverfahren, die Lichtimpulse aussenden, deren Laufzeit bis zur Reflexion messen und so Entfernungen sowie Strukturen in drei Dimensionen rekonstruieren \parencite{mcmanamon2015fieldguide}. Nach frühen Anwendungen in Geodäsie und Fernerkundung seit den 1960er-Jahren ermöglichte der Fortschritt in Lasertechnik und Miniaturisierung die Übertragung in die Automobilindustrie, wo \ac{LiDAR} heute eine Schlüsselrolle für hochautomatisierte Funktionen einnimmt \parencite{beuth2021handbuch}.

Im Sensorsystemverbund liefert \ac{LiDAR} somit die geometrische Präzision, mit der Kamera- und Radarinformationen redundant oder ergänzend genutzt werden können. Diese Rolle macht die Technologie besonders wertvoll für Aufgaben wie Hinderniserkennung, Bahnplanung und eine robuste Umfeldrepräsentation. Auf dieser Grundlage werden im Folgenden Sensoraufbau, Wellenlängenbereiche und Messprinzip der Ouster-Plattform vorgestellt, die die experimentelle Basis der Arbeit bilden.
...
Die gewählte Detektionskette reagiert empfindlich auf Parameter, die Segmentierung, Clusterbildung und Klassentrennung prägen. Die Parameterstudie untersucht deshalb systematisch die Kombinationen aus \textit{voxel_size}, \textit{distance_threshold} und \textit{cluster_tolerance}, indem pro Durchgang jeweils nur einer dieser Werte variiert und die restliche Konfiguration unverändert belassen wird. So bleibt der Einfluss jeder Stellgröße klar nachvollziehbar.
