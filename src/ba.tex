...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|}
    ...
    \texttt{x, y, z} & Kartesische Koordinaten in Metern im \texttt{frame\_id} (Sensor- oder Fahrzeugrahmen). \newline \emph{Koordinatensystem:} rechtsdrehend gemäß REP~103, d.\,h. \(x\) zeigt vorwärts, \(y\) nach links, \(z\) nach oben. \\ \hline
    \texttt{intensity} & Reflektierte Signalstärke ohne physikalische Einheit; vom Treiber auf \texttt{uint16} skaliert. Kalibrierung und automatische Verstärkung sind sensorspezifisch, daher sind Werte nur innerhalb eines Scans zuverlässig vergleichbar. \\ \hline
    \texttt{ring} & Vertikaler Kanalindex des LiDAR (Laser/Detektor-Paar). Beim eingesetzten OS1-64: \(64\) diskrete Ringe von \(0\) (unterster Strahl) bis \(63\) (oberster Strahl); andere Modellvarianten besitzen entsprechend weniger oder mehr Ringe. \\ \hline
    \texttt{time} & Zeitversatz eines Punktes relativ zu \texttt{header.stamp} in Sekunden; dient der intra-scan Motion Compensation. \\ \hline
  \end{tabular}
...
\begin{enumerate}
  ...
  \item Start des zentralen Launch-Files (\texttt{ros2 launch ...}) als Hintergrundprozess; über \texttt{trap} wird eine saubere Terminierung (\texttt{SIGINT}/\texttt{SIGTERM}) gewährleistet und \texttt{ros2 bag record} optional parallel aktiviert.
...
Die Auswahl der Grenzen folgt in dieser Arbeit einem systematischen Verfahren: Zunächst wurde eine großzügige Box \(\mathbf{b}_\text{min}=[-20,-15,-3]^\top\), \(\mathbf{b}_\text{max}=[20,15,5]^\top\) definiert, um alle relevanten Rückläufer sicher zu erfassen. Anschließend erfolgte eine schrittweise Schrumpfung entlang \(x\), \(y\) und \(z\) in \(0{,}5\,\mathrm{m}\)-Schritten. Bewertet wurde jede Konfiguration anhand (i) Detektionspräzision nach dem Clustering (Anteil falsch positiver Objekte), (ii) verbleibender Punktzahl sowie (iii) Anteil abgeschnittener Fahrbahnränder in RViz. Dieses Vorgehen entspricht der iterativen ROI-Abgrenzung, wie sie in LiDAR-Pipelines zur Falsch-Alarm-Reduktion empfohlen wird \cite{hu2013robust,himmelsbach2010fast}. Die finalen Grenzen \(\mathbf{b}_\text{min}=[-10,-6,-3]^\top\) und \(\mathbf{b}_\text{max}=[10,6,5]^\top\) lieferten den besten Kompromiss aus reduzierter Punktzahl (\(-57\,\%\)) und stabiler Objektabdeckung im urbanen Messfeld.
...
\subsection{Verifikation und Tests}
...
\lstinputlisting[language=C++,caption={C++-Implementierung des \texttt{crop\_box\_node} in ROS~2},label=lst:cropbox_node]{ouster_cpp/src/crop_box_node.cpp}
