...
% ---------- Metadata ----------
\newcommand{\arbeitstitel}{Entwicklung und Evaluierung von Algorithmen zur Umfelderkennung auf Basis hochauflösender 360°-\ac{LiDAR}-Sensordaten}
...
\noindent
Die vorliegende Arbeit entwickelt eine modular aufgebaute Verarbeitungsstrecke zur \ac{LiDAR}-basierten Umfelderkennung für das Projektautomobil \textit{CarCeptionX}. Ausgangspunkt ist die bestehende \ac{ROS}\,2-Messkette des Ouster~\ac{OS1}, die im Rahmen dieser Arbeit um eine robuste Folge von Vorverarbeitungs-, Segmentierungs-, Cluster- und Trackingbausteinen erweitert wird. Ziel ist eine echtzeitfähige Erkennung relevanter Verkehrsteilnehmer ab einer Mindestgröße von \SI{0.5}{\meter} bei mindestens \SI{90}{\percent} Detektionsrate bei \SI{10}{\hertz} Ausführung.

Das System umfasst eine rausch- und ausreißerreduzierende Vorverarbeitung, eine szenarienunabhängige Bodensegmentierung, die Bildung konsistenter Cluster mit zugehörigen Bounding-Boxen sowie ein darauf aufbauendes Objekt-Tracking. Sämtliche Schritte werden in \ac{ROS}~2 implementiert, mit einheitlichen Schnittstellen versehen und hinsichtlich Ausführungsfrequenz, Latenz und Speicherbedarf untersucht. Für zentrale Algorithmen wie \ac{RANSAC}-Bodenschätzung, Euclidean-Cluster-Extraction, \ac{DBSCAN} und \ac{CV}-Kalman-Filter werden Parameterstudien durchgeführt, um die Balance zwischen Genauigkeit und Rechenaufwand zu optimieren.

In realen Messszenarien auf urbanen Teststrecken zeigt die Pipeline stabile Resultate: Die Bodensegmentierung trennt befahrbare Fläche und Hindernisse verlässlich, die Clusterbildung erzeugt weitgehend trennscharfe Objektboxen, und das Tracking liefert kohärente Trajektorien auch bei kurzzeitigen Abschattungen. Die Ausführungsraten liegen auf handelsüblicher Hardware im Bereich von \SIrange{10}{15}{\hertz}, womit die geforderten Echtzeitanforderungen erfüllt werden. Die Arbeit schließt mit einer Bewertung der erzielten Leistung und einem Ausblick auf Erweiterungen wie Sensorfusion, semantische Segmentierung und eine engere Integration in das Fahrzeugsystem.
...
Dieses Kapitel bündelt die wissenschaftlichen Grundlagen der \ac{LiDAR}-Sensorik und ordnet sie in den automobilen Kontext ein. Es stellt die Messprinzipien und den historischen Entwicklungspfad von \ac{LiDAR} vor, grenzt die Technologie gegenüber Kamera und Radar ab, erläutert den Aufbau des verwendeten Sensors und bereitet damit die theoretische Basis für die in den Folgekapiteln entwickelte Verarbeitungskette.
...
hat sich insbesondere die \ac{LiDAR}-Sensorik (\emph{Light Detection and Ranging}) 
als Schlüsseltechnologie etabliert (\cite{beuth2021handbuch}).

Das Akronym \emph{\ac{LiDAR}} steht für „Light Detection and Ranging“ und 
...
Streuungsprozesse erfassen. \ac{LiDAR}-Systeme senden kurze Lichtimpulse 
...
Die Entwicklung von \ac{LiDAR} begann in den 1960er-Jahren, zunächst für 
...
Miniaturisierung optoelektronischer Komponenten fand die \ac{LiDAR}-Sensorik 
...
jedoch nur über eine eingeschränkte Winkelauflösung. \ac{LiDAR} kombiniert die 
...
Im Kontext des autonomen Fahrens ermöglicht \ac{LiDAR} eine 
präzise Abbildung des Umfelds, die sowohl für die Hinderniserkennung 
als auch für die simultane Lokalisierung und Kartierung (\ac{SLAM}) genutzt 
werden kann. Aufgrund seiner Eigenschaften wird \ac{LiDAR} häufig als 
...
Ein \ac{LiDAR}-Sensor umfasst Sendeoptik und Strahlablenkung, Empfangsoptik und Lichtsensor sowie Auslese- und Steuerungselektronik (vgl. Abb.~\ref{fig:sensoraufbau}).
...
Als Detektoren kommen für automotive \ac{LiDAR}-Systeme typischerweise 
...
Dieses Kapitel skizziert die technische Systemarchitektur der \ac{LiDAR}-basierten Umfelderkennung und ordnet alle Software- und Hardwarekomponenten in eine durchgängige \ac{ROS}~2-Verarbeitungskette ein. Es erläutert die eingesetzten Frameworks und Bibliotheken, beschreibt die Arbeitsumgebung sowie die Datenaufnahme und -verarbeitung, geht auf das Deployment der Nodes ein und definiert die Bewertungsmetriken, anhand derer die Pipeline in den folgenden Kapiteln verifiziert wird.

\section{Überblick über das Gesamtsystem}
\subsection{Anforderungen an den Algorithmus}

Die Punktwolken des Ouster~\ac{OS1} bilden die Grundlage einer Verarbeitungskette, die relevante Objekte im Umfeld extrahiert und nachverfolgt. Die hohe Winkelauflösung des Sensors ermöglicht eine präzise Rekonstruktion lokaler Geometrien \parencite{OusterOS1}, erfordert jedoch eine robuste Vorverarbeitung: Boden- und Störanteile müssen entfernt, die Punktdichte kontrolliert reduziert und gleichzeitig ausreichende Detailinformationen für verlässliche Bounding Boxes bewahrt werden.
...
Um den Einsatz auf Standardhardware zu ermöglichen und die Reproduzierbarkeit sicherzustellen, wird eine End-to-End-Latenz unter \(100\,\text{ms}\) bei \(10\,\text{Hz}\) angestrebt, verbunden mit moderater CPU/RAM-Auslastung und klar definierten \ac{ROS}~2-Nachrichten für maschinen- wie terminallesbare Ausgaben. Standardhardware meint hier einen mobilen Rechner oder Embedded-PC mit mindestens 6 CPU-Kernen (\(\geq 2{,}5\,\text{GHz}\)), \(\geq 16\,\text{GB}\) RAM und ohne dedizierte GPU-Anforderungen. Die Einbindung in die bestehende GUI des Projekts \emph{Carception~X} \parencite{IFZN_Projekte} stellt darüber hinaus Kompatibilität mit den vorhandenen Betriebsmodi (Sensor, Sensor + PC, Sensor + PC + Embedded PC) sicher \parencite{Wendel2025}.

Die Güte des Bodenfilters wird anhand zweier Größen bewertet: (1) Restbodenanteil, gemessen als Anteil verbleibender Bodenpunkte je Rasterzelle (\(2\,\text{m}\) \(\times\) \(2\,\text{m}\)) unter \(5\,\%\), (2) Verlust tragender Objektpunkte, gemessen als Reduktion der Stützpunkte innerhalb eines \(30\,\text{m}\) Frontbereichs um höchstens \(10\,\%\). "Ohne wesentlichen Geometrieverlust" bedeutet, dass die Punktdichte nach Downsampling mindestens \(70\,\%\) der Ausgangsdichte innerhalb eines \(3\,\text{m}\) \(\times\) \(3\,\text{m}\)-Gitters beträgt und die Bounding-Box-Kantenlänge maximal \(0{,}1\,\text{m}\) vom Referenzwert abweicht. Anpassbare Laufzeitparameter sind insbesondere Voxelgröße, \ac{RANSAC}-Abstandsgrenze und Iterationen für die Bodenebene, Cluster-Toleranz sowie Minimal-/Maximalpunktzahlen pro Cluster; sie werden über Launch-Dateien oder \texttt{ros2 param set} verändert und ohne Neustart übernommen. Eine Echtzeitvisualisierung (z.\,B. in RViz2 oder der GUI) muss Rohdaten, Bodenfilter, Cluster und Tracks bei mindestens \(10\,\text{\ac{FPS}}\) sichtbar machen.
...
Abbildung~\ref{fig:ros2-pipeline-alltopics} zeigt die \ac{ROS}~2-Verarbeitungskette von der Aufnahme über Vorverarbeitung, Bodensegmentierung und Cluster/Bounding-Boxes bis zur Objektverfolgung. Jeder Knoten hat klar definierte Ein-/Ausgänge und Topics.
...
Außerdem wird ein Python Skript ausgeführt zur Berechnung der CPU Auslastung pro Node (resource\_probe.py)
...
Dieses Kapitel konzentriert sich auf die Vorverarbeitung der \ac{LiDAR}-Punktwolken, um Datenvolumen und Rauschen vor den nachfolgenden Schritten zu reduzieren. Es beschreibt die verwendeten Filter zur räumlichen Begrenzung und Ausdünnung der Punktwolke sowie ihre Parametrierung, sodass die späteren Segmentierungs- und Clusteringverfahren auf einer robusten und gleichmäßig verteilten Datenbasis aufsetzen können.
...
Die Auswahl der Grenzen folgt in dieser Arbeit einem systematischen Verfahren: Zunächst wurde eine großzügige Box \(\mathbf{b}_\text{min}=[-20,-15,-3]^\top\), \(\mathbf{b}_\text{max}=[20,15,5]^\top\) definiert, um alle relevanten Rückläufer sicher zu erfassen. Anschließend erfolgte eine schrittweise Schrumpfung entlang \(x\), \(y\) und \(z\) in \(0{,}5\,\mathrm{m}\)-Schritten. Bewertet wurde jede Konfiguration anhand (i) Detektionspräzision nach dem Clustering (Anteil falsch positiver Objekte), (ii) verbleibender Punktzahl sowie (iii) Anteil abgeschnittener Fahrbahnränder in \ac{RViz}. Dieses Vorgehen entspricht der iterativen \ac{ROI}-Abgrenzung, wie sie in \ac{LiDAR}-Pipelines zur Falsch-Alarm-Reduktion empfohlen wird \cite{hu2013robust,himmelsbach2010fast}. Die finalen Grenzen \(\mathbf{b}_\text{min}=[-10,-6,-3]^\top\) und \(\mathbf{b}_\text{max}=[10,6,5]^\top\) lieferten den besten Kompromiss aus reduzierter Punktzahl und stabiler Objektabdeckung im urbanen Messfeld.

\subsection{Implementierung}
Im Rahmen dieser Arbeit wurde der \texttt{crop\_box\_node} in C++ unter \ac{ROS}~2
entwickelt, um diesen Filter dynamisch innerhalb der gesamten Verarbeitungskette anzuwenden.
Der Node empfängt eine eingehende Punktwolke vom Ouster-\ac{LiDAR}-Sensor auf dem Topic
...
Der \texttt{crop\_box\_node} wurde in C++ unter \ac{ROS}~2 implementiert und nutzt die \ac{PCL}-Schnittstellen, um die Ungleichung aus Gleichung~\eqref{eq:cropbox} Punkt für Punkt anzuwenden. Wichtige Kernschritte sind: Laden der Parameter \texttt{min\_bound} und \texttt{max\_bound}, Subscription des Eingangstopics \texttt{/ouster/points} mit SensorDataQoS und Publikation des Ergebnisses auf \texttt{/points\_cropped}. Ein Auszug zeigt den wesentlichen Ablauf \cite{pcl_docs_2025}:
...
und folgt der in \ac{ROS} etablierten Konvention nach REP~103: \(x\) zeigt in Fahrtrichtung,
\(y\) nach links und \(z\) nach oben; der Ursprung liegt im \ac{LiDAR}-Frame auf Fahrzeughöhe.
...
die \ac{ROS}~2-Verbindungen neu aufgebaut werden müssen.
...
die bei der Verarbeitung von \ac{LiDAR}-Daten in urbanen Umgebungen besonders relevant sind.  
...
Die Boxermittlung folgt auf die Punktzusammenfassung und nutzt ausschließlich Clusterpunkte.
...
Vorteile: sehr geringe Rechenkosten, deterministisch und robust gegenüber Ausdünnung/Teilsicht. Nachteil: bei gedrehten Objekten überschätzt die AABB die Grundfläche in der Draufsicht (\ac{BEV}), da die Orientierung nicht erfasst wird.
...
\begin{itemize}
  \item \textbf{AABB}: sehr schnell, stabil, deterministisch; \emph{−} keine Orientierung, tendenziell größere Grundfläche bei gedrehten Objekten.
  \item \textbf{OBB}: Orientierung/kompaktere Hülle bei elongierten Objekten; \emph{−} aufwändiger, empfindlicher gegenüber Ausdünnung/Teilsicht, potenziell sprunghaft.
\end{itemize}

In dieser Arbeit werden beide Methoden getestet und verglichen und \textbf{AABB} ausgewählt. 
\begin{figure}[H]
  \centering
...
Hauptgründe sind die hohe Stabilität der Boxen unter realen Sensorbedingungen (variable Punktdichte, Okklusionen) und die geringen Rechenkosten, die die Echtzeitfähigkeit in der Gesamtkette begünstigen. Die Orientierung kann optional aus nachfolgenden Schritten (z.\,B. in \ac{BEV}) geschätzt werden, ohne die Boxbildung zu destabilisieren. Eine OBB-Erweiterung ist als künftige Verbesserung vorgesehen, insbesondere wenn robuste Orientierungsmerkmale (z.\,B. \ac{BEV}-MinArea) zuverlässig verfügbar sind.
Abbildung~\ref{fig:aabb_obb_compare} zeigt den praktischen Unterschied.
...
Dieses Kapitel erläutert die Methoden zur zeitlichen Verfolgung der in der Clusterstufe detektierten Objekte. Es beschreibt den eingesetzten zweidimensionalen Kalman-Filter samt Bewegungsmodell, die Mess- und Zuordnungslogik mit Ungarischem Algorithmus sowie das Track-Management und schließt mit den Tests und Ergebnissen zur Stabilität der Track-IDs.
...
\texttt{ids\_probe.py} beobachtet das Marker-Topic \texttt{tracks\_markers} und führt ein Lebensdauerkonto pro Track-ID: Geburt und Abschluss (bei Leerlauf länger als \texttt{idle\_timeout}) werden gezählt, aktive und abgeschlossene mittlere Lebensdauern berechnet und die Geburts- bzw. Abschlussrate pro Minute ausgegeben. Der Skript läugt als eigene \ac{ROS}~2-Nodes, greift auf die bestehenden Topics zu und unterstützt damit die Stabilität der Track-IDs.

Ergebnisse der Verifikation der Objektverfolgung sind ~ref{label:gerade_strecke} zu entnehmen.
...
Dieses Kapitel stellt die Testergebnisse der \ac{ROS}~2-basierten Verarbeitungskette vor und bewertet sie anhand der definierten Anforderungen. Es beschreibt den Versuchsaufbau und die Parametervariationen, fasst die Leistungskennzahlen zu Latenz, Ressourcenbedarf und Objekterkennung zusammen und diskutiert, wie die Ergebnisse den angestrebten Zielwerten gegenüberstehen. Die Verarbeitungskette wurde vollständig in \ac{ROS}~2 umgesetzt und besteht aus den in Abbildung~\ref{fig:ros2-pipeline-alltopics} gezeigten Modulen; die Konfiguration erfolgt über das Launch-File \textit{benchmark.launch.py}, welches alle Module innerhalb des gemeinsamen Namensraums \textit{/bench} startet.

\section{Parameterstudie}
\label{sec:parameterstudie}

Die gewählte Detektionspipeline ist sensitiv gegenüber mehreren Parametern, die die Qualität der Segmentierung, Clusterbildung und Klassentrennung maßgeblich beeinflussen. Entsprechend untersucht die Parameterstudie, welche Kombinationen aus \textit{voxel\_size}, \textit{distance\_threshold} und \textit{cluster\_tolerance} in unterschiedlichen urbanen Szenarien robuste Ergebnisse liefern. Die ROS2-Bagfiles wurden dazu mehrfach wiederholt abgespielt; pro Durchgang änderte sich nur ein Parameter, während alle anderen konstant blieben. So lässt sich der isolierte Einfluss jeder Stellgröße bewerten und gezielt gegenrechnen.

Die Datensätze für die Parametrisierung wurden bei 9\,°C und bedecktem Himmel aufgezeichnet, sodass keine direkten Wettereinflüsse durch starke Sonneneinstrahlung oder Niederschlag zu erwarten waren. Verarbeitet wurden sie auf einem MEDION 15~E1 Laptop mit AMD Ryzen~5~7430U (6~Kerne/12~Threads) und 32~GB RAM.

Im Fokus stehen bewusst nur diese drei Stellgrößen, weil sie die größten Hebel auf die Verarbeitungsqualität besitzen und jeweils unterschiedliche Stufen der Pipeline adressieren: \textit{voxel\_size} steuert die Ausdünnung der Punktwolke (Rauschunterdrückung versus Detailtreue), \textit{distance\_threshold} legt die Inlier-Schwelle der Bodensegmentierung fest und \textit{cluster\_tolerance} bestimmt die räumliche Nachbarschaft beim Clustering. Andere Parameter wurden nicht variiert, da sie entweder stark durch die Versuchskonfiguration vorgegeben sind (\texttt{min\_bound}/\texttt{max\_bound} der CropBox definieren den vermessenen Sichtbereich), bereits konservative Robustheitsreserven bieten (\texttt{max\_iterations} der \ac{RANSAC}-Suche), nur sekundären Einfluss auf die Klassentrennung haben (\texttt{min\_cluster\_size}, \texttt{max\_cluster\_size}, \texttt{max\_clusters}) oder die Stabilität der Track-IDs betreffen und damit die Vergleichbarkeit der Wiederholungsmessungen erschweren würden (\texttt{gate\_dist\_max}, \texttt{max\_missed}, \texttt{min\_hits}). Auf diese Weise bleibt der Suchraum handhabbar, ohne zentrale Qualitätstreiber der Pipeline zu vernachlässigen.
...
Die Auswertungen für \textbf{S1} zeigen deutlich, wie sensibel die Pipeline auf Übersegmentierung reagiert. Eine Vergrößerung der \texttt{voxel\_size} von \SI{0.10}{m} auf \SI{0.20}{m} reduziert die Fehlklassifikationen von Menschen und Fahrradfahrern vollständig und nähert sich mit insgesamt 17 Detektionen dem Realwert von 19 Objekten an. Gleichzeitig verhindert die feinere Bodenschätzung mit einem \texttt{distance\_threshold} von \SI{0.20}{m}, dass Fahrzeugdachpunkte als Boden eingestuft werden; dies steigert die Gesamtdetektionen auf 20, jedoch um den Preis eines zusätzlichen LKW-Falschpositivs. Am deutlichsten wird der Einfluss der \texttt{cluster\_tolerance}: Zu kleine Werte (\SI{0.30}{m}) führen zu einer Übersegmentierung von PKW (24 Gesamt\-detektionen), während zu große Werte (\SI{0.70}{m} und \SI{0.90}{m}) angrenzende Fahrzeuge zusammenziehen (nur 11 bzw. 10 Detektionen). Die Einstellung \SI{0.50}{m} liefert mit 19 Detektionen eine nahezu perfekte Übereinstimmung mit der Bodenwahrheit und stellt damit den besten Kompromiss zwischen Trennschärfe und Zusammenfassung dar.
...
Die Ergebnisse der Szene \textbf{S2~–~Kreuzung} zeigen, dass eine dichte und dynamische Umgebung die Wirkung jedes Parameters verstärkt.
Eine moderate Reduzierung der \textit{voxel\_size} (0,15--0,20\,m) senkt die Falschmeldungen in der Klasse \textit{Mensch} (von 13 auf 2~Detektionen),
führt aber gleichzeitig zu einer leichten Untererfassung der tatsächlichen Fußgänger (3 erwartete Ziele) und weiterer Klassen \big(\autoref{tab:kreuzung_voxel_size}\big).
Ein \textit{distance\_threshold} von 0,15--0,20\,m verhindert eine Explosion von Fehl-Clustern, ohne große Objekte zu verlieren; geringere Werte (0,10\,m)
bewirken eine Übersegmentierung und fast doppelt so viele Detektionen wie real vorhanden \big(29 statt 14, \autoref{tab:kreuzung_distance_threshold}\big).
Die Variation der \textit{cluster\_tolerance} belegt, dass ein Umfeld von 0,50--0,70\,m einen guten Kompromiss bietet: darunter (0,30\,m) werden Objekte übersegmentiert (21~Detektionen, davon 4~falsche Radfahrer),
darüber (0,90\,m) beginnen sie zu fusionieren (26~Detektionen, 7 über dem Realwert), siehe \autoref{tab:kreuzung_cluster_tolerance}.
In Summe bleiben die gewählten Einsatzwerte (\textit{voxel\_size} = 0,15--0,20\,m, \textit{distance\_threshold} = 0,15\,m, \textit{cluster\_tolerance} = 0,50\,m)
eine robuste Wahl, um Fehlalarme zu begrenzen und gleichzeitig die wesentlichen Ziele in einer stark frequentierten Kreuzung zu erhalten.
...
\begin{enumerate}
    ...
    \item \textbf{Innenstadt mit Steigung, 30\,km/h:} Leichte Steigungsfahrt in urbanem Umfeld. Prüft, ob die Pipeline auch bei
...
Eingangsfrequenzen führen, ohne jedoch die Verarbeitungslogik der Pipeline selbst zu
...
Messwerte, dass die Pipeline auch unter variierenden Umweltbedingungen performant arbeitet und genügend Reserven für
...
Insgesamt erfüllt die entwickelte Pipeline die definierten funktionalen und nicht-funktionalen Anforderungen: Sie arbeitet echtzeitfähig, ressourcenschonend und robust gegenüber wechselnden Umgebungsbedingungen. Optimierungspotenzial besteht vor allem im Feintuning der Tracking-Schwellen, um kurzlebige Track-Geburten in statischen Szenen weiter zu reduzieren, ohne die Reaktionsfähigkeit in dynamischen Situationen zu beeinträchtigen.

% ====== 8 Zusammenfassung und Ausblick ======
\chapter{Zusammenfassung und Ausblick}
\section{Zusammenfassung}
Die vorliegende Arbeit entwickelte eine \ac{LiDAR}-basierte Verarbeitungspipeline, die alle Schritte von der Vorverarbeitung bis zur
...
bestätigte die Pipeline ihre Echtzeitfähigkeit: Die Verarbeitungslatenzen blieben im typischen Fahrbetrieb deutlich unter
...
Parametrierung der Filter- und Segmentierungsstufen an, um die Pipeline dynamisch an unterschiedliche Wetter- oder
...
\begin{itemize}
  ...
  \item verwendete Launch-Dateien (z.\,B. \texttt{benchmark.launch.py},
        \texttt{ouster\_pipeline.launch.py})
\end{itemize}
